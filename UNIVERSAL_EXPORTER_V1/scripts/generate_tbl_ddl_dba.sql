...
select DECODE((select count(*) from dba_tables WHERE owner = UPPER('&2') AND table_name = UPPER('&3')), 1, ('&1' || '\\'), 0, ('&1' || '\\' || 'NOT_EXISTS_')) || REPLACE('&3', '$', '_dollar') || '.' || lower(trim('&4')) fname from dual;
spool &fname
