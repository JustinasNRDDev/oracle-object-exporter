...
Sitas aplankas skirtas gyvam `UNIVERSAL_EXPORTER_V2` (BAT/task runtime) testavimui su lokaliu Oracle 19c konteineriu.
...
- `TASK_LOCAL19/objects.txt` - task-mode objektu sarasas V2 vykdymui su aplinka `LOCAL19`.
- `setup_objects.ps1` - paleidzia testiniu objektu sukurima.
- `run_live_export.ps1` - paleidzia gyva eksporta per `oracle_exporter_task.bat` ir `TASK_LOCAL19`.
...
```powershell
$task = (Resolve-Path .\testing\local_oracle19\TASK_LOCAL19).Path
.\oracle_exporter_task.bat -TaskName "$task" -EnvironmentName LOCAL19 -ConfigPath .\config\exporter.yaml -DryRun
```
...
```text
testing/local_oracle19/TASK_LOCAL19/LOCAL19/<timestamp>/APPUSER19/...
testing/local_oracle19/TASK_LOCAL19/LOCAL19/<timestamp>/APPUSER19_2/...
```
