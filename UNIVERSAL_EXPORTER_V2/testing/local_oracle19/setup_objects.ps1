...
try {
    sqlplus $connection @testing/local_oracle19/sql/setup_test_objects.sql
    if ($LASTEXITCODE -ne 0) {
        throw "setup_test_objects.sql baigėsi su klaida (code=$LASTEXITCODE)."
    }
}
finally {
    Pop-Location
...
Write-Host "Testiniai objektai sukurti sėkmingai."
