...
select
case 
   when t.type = 'TYPE' and t.line= 1 then
...
case 
   ...
   when
      ...
      s.text
end mod from all_source s where s.owner='&3' and s.name='&2' and s.type='TYPE') t order by t.line;
prompt /

select
case 
   when t.type = 'TYPE BODY' and t.line = 1 then
...
case 
   ...
   when
      ...
      s.text
end mod from all_source s where s.owner='&3' and s.name='&2' and s.type='TYPE BODY') t order by t.line;
prompt /
