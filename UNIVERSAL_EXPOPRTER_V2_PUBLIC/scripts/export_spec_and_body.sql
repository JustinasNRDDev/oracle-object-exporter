...
select
case 
   when t.type = 'PACKAGE BODY' and t.line = 1 then
     'CREATE OR REPLACE ' || REGEXP_REPLACE(t.mod, '(\S+) (\S+)\s+(\S+)', '\1 \2 \3')
   when t.type = 'PACKAGE' and t.line= 1 then
...
case 
   ...
   when
      ...
      s.text
end mod from all_source s where s.owner='&3' and s.name='&2' and s.type='PACKAGE') t order by t.line;

prompt /
prompt show errors PACKAGE &3..&2
prompt
select
case 
   when t.type = 'PACKAGE BODY' and t.line = 1 then
...
case 
   ...
   when
      ...
      s.text
end mod from all_source s where s.owner='&3' and s.name='&2' and s.type='PACKAGE BODY') t order by t.line;
